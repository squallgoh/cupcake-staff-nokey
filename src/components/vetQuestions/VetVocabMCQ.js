...
import { QT_VOCAB_MCQ_ID, ENGLISH_SUBJECT_ID } from "../questions/_config.json";
...
const VetVocabMCQ = () => {
      ...
      try {
        const { data } = await API.graphql(
          graphqlOperation(getSkillGroup, {
            id: "e7866666-db74-4005-8568-609ac6cd2f72",
          })
        );
        const vocabSkills = data.getSkillGroup.skills.items;
        const vocabSubSkills = vocabSkills.map((skill) => {
        ...
        setSubSkills(vocabSubSkills.flat());
      } catch (e) {
        console.error(e);
      }
    };
  ...
  const renderExplanationEditor = () => {
    if (currentQuestion) {
      const namePrefix = `vocabMCQ/${currentQuestion.id}/explanation-`;
  ...
  const renderEditor = () => {
    if (subject !== undefined && questionsCache.length) {
      return (
        <Container>
          <Header>Vet Vocabulary MCQs</Header>
